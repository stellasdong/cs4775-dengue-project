...
def main():
    ...
    D, mapping = a.read_data(distances_file)
    og = dict(map(reversed, mapping.items()))[og_]

    E, uD, fake_root = a.neighbor_join(D, og) # TIP: Please change the arguments to pass and return here if you don't want to follow the provided structure
    mapping[fake_root] = "root"
    tree_map = a.assemble_tree(fake_root, E) # TIP: Please change the arguments to pass and return here if you don't want to follow the provided structure
    nwk_str = a.generate_newick(fake_root, tree_map, uD, mapping) # TIP: Please change the arguments to pass and return here if you don't want to follow the provided structure
    
    tree = Phylo.read('tree.nwk', 'newick')

    # Root the tree using Green_monkey
    tree.root_with_outgroup("Green_monkey")
